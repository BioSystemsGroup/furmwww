...
- getLinkList
{
  id <List> linkList = [List create: scratchZone];
  id <ListIndex> nodeNdx = [nodeList listBegin: scratchZone];
  id link=nil, node=nil;
  while ( ([nodeNdx getLoc] != End)
          && ( (node = [nodeNdx next]) != nil) ) {
    id <ListIndex> linkNdx = [[node getToLinks] listBegin: scratchZone];
    while ( ([linkNdx getLoc] != End)
            && ( (link = [linkNdx next]) != nil) ) {
...
{
  id <ListIndex> nodeNdx = [[self getNodeList] listBegin: scratchZone];
  id <DiGraphNode> dgn = nil;
  id <DiGraphNode> retNode = nil;

  while ( ([nodeNdx getLoc] != End)
          && ( (dgn = [nodeNdx next]) != nil) ) {
    if ([dgn getNumber] == nodeID) {
      retNode = dgn;
      break;
    }
  }
...
- (id <DiGraphNode>) findNodeWithLabel: (const char *) nodeLabel
{
  ...
  while ( ([nodeNdx getLoc] != End)
          && ( (dgn = [nodeNdx next]) != nil) ) {
    if (strcmp([dgn getNodeLabel], nodeLabel) == 0) {
      retNode = dgn;
      break;
    }
  }
